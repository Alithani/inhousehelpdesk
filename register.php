...
//if ($_SERVER["REQUEST_METHOD"] == "POST"){
  if(isset($_POST['signup']))
  {
      ...
      else  if($num_rows_update==0) 
      {
        //image upload
        $target_dir = "images/";
        $target_file = $target_dir.$fname.basename($_FILES["avatar"]["name"]);
        $uploadOk = 1;
        $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
        $new_name = $target_dir.$fname.basename($_FILES["avatar"]["name"]);        
        ...
        $avatarimg = $fname.basename($_FILES["avatar"]["name"]);
